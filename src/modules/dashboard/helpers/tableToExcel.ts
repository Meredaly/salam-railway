  ...
  return function ({ head, body, name, fileName }: { head: any; body: any; name: string; fileName: string }) {
    ...
    body.forEach((bodyItem: any, index: number) => {
      ...
      head.forEach((headItem: any) => {
        ...
        if (headItem.key === 'dayForRepair') {
          td.style.background =
            bodyItem.dayForRepair < 0
              ? '#ff7171'
              : bodyItem.dayForRepair <= 30 && bodyItem.dayForRepair > 0
              ? '#fdfd52'
              : bodyItem.dayForRepair > 30
              ? '#72ff75'
              : ''
        ...
        if (headItem.visibility) {
          ...
          td.append(bodyItemValue?.toString() || '')
          tr.append(td)
        }
      })
      table.append(tr)
